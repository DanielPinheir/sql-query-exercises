...
-- 5 - Selecione as composições e o tempo de cada composição,
-- de todos os registros da tabela musicas, onde o compositor 
-- seja Mozart ou Bach.
SELECT compositor, composicao, tempo FROM musicas 
WHERE compositor ILIKE 'mozart' OR compositor ILIKE 'bach';
...
-- e faça uma ordenação pelo tempo de execução de cada música,
...
-- de forma que esses 10 registros sejam os que tenham menor tempo de execução
-- com exceção dos 5 primeiro mais rápidos.
SELECT * FROM musicas ORDER BY tempo ASC LIMIT 10 OFFSET 5;

-- 11 - Seguindo o conceito de paginação de resultados, sabendo que cada página
-- retorna apenas 10 resultados, faça a listagem dos registros que correspondam 
-- a página 4.
SELECT * FROM musicas ORDER BY id ASC LIMIT 10 OFFSET 10 * 3;

-- 12 - Seguindo o conceito de paginação de resultados, 
-- sabendo que cada página retorna apenas 12 resultados, 
-- faça a listagem dos registros que correspondam a página 6.
SELECT * FROM musicas ORDER BY id ASC LIMIT 12 OFFSET 12 * 5;

-- 13 - Selecione todos os compositores da tabela musicas de forma
-- que os resultados não sejam repetidos e não esteja nulo.
SELECT DISTINCT(compositor) AS nome_do_compositor 
FROM musicas WHERE compositor IS NOT NULL;
...
-- 15 - Selecione todos os campos da tabela musicas e os registros 
-- que o nome do compositor comece exatamente com Bra, isso significa 
-- que se a primeira letra não form maiúscula está incorreto.
SELECT * FROM musicas WHERE compositor LIKE 'Bra%';

-- 16 - Selecione todos os campos da tabela musicas e os registros que
-- o ritmo possua a última palavra igual a troppo independente do restante
-- do texto que antecede, mas que seja exatamente a última palavra do texto.
SELECT * FROM musicas WHERE ritmo LIKE '%troppo';

-- 17 - Faça uma busca nos registros da tabela musicas e
-- retorne somente os registros que possua em qualquer posição do texto,
...
-- 18 - Faça uma busca nos registros da tabela musicas e retorne somente
-- os registros que não possua a palavra quintet na coluna composição, 
